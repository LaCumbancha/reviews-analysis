...
func NewJoiner(config JoinerConfig) *Joiner {
	...
	inputDirect1 := rabbitmq.NewRabbitInputDirect(props.DishashFilterOutput, config.InputTopic, ch)
	inputDirect2 := rabbitmq.NewRabbitInputDirect(props.BotUsersFilterOutput, config.InputTopic, ch)
	outputQueue := rabbitmq.NewRabbitOutputQueue(props.BotUsersJoinerOutput, config.Instance, ch)
	joiner := &Joiner {
		connection:		conn,
		channel:		ch,
