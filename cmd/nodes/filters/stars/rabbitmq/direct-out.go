...
func (direct *RabbitOutputDirect) PublishData(bulkNumber int, starsDataList []StarsData) {
	dataListByPartition := make(map[string][]StarsData)

	for _, data := range starsDataList {
		...
		if partition != "" {
			...
			if starsDataListPartitioned != nil {
				...
				dataListByPartition[partition] = append(make([]StarsData, 0), data)
			}
	...
	for partition, starsDataListPartitioned := range dataListByPartition {
		outputData, err := json.Marshal(starsDataListPartitioned)

		if err != nil {
			log.Errorf("Error generating Json from (%s). Err: '%s'", starsDataListPartitioned, err)
		} else {
